...
//             ui->wdg_jpg , SLOT(slot_setImage(QImage)) );
   ...
   // m_recTimer->start( 500 );
    ...
    ui->lb_gif->installEventFilter( this );
}
...
//过滤器
bool UploadDialog::eventFilter(QObject *watch, QEvent *event)
{
    if( watch == ui->lb_gif && event->type() == QEvent::Enter)
    {
        if( ui->lb_gif->movie() )
            ui->lb_gif->movie()->start();
    }
    if( watch == ui->lb_gif && event->type() == QEvent::Leave)
    {
        if( ui->lb_gif->movie() )
            ui->lb_gif->movie()->stop();
    }
...
//    0    tst3    id 10  --> rtmp地址    rtmp://服务器ip/appName
    ...
    QString rtmpUrl = QString("rtmp://%1/videotest").arg(OnlineDialog::m_ServerIP);
    m_savePath = rtmpUrl;
...
//回看功能
void UploadDialog::on_pb_see_clicked()
{
    if( ui->lb_res->text().remove(" ").isEmpty() ) return;
    QString str = ui->lb_res->text().remove("录制成功");
    if(str.contains("rtmp"))
    {
        QMessageBox::about( this , "提示", "直播无法回看");
        return;
    }
    emit SIG_PlayRecord(str);
}
