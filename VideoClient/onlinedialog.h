...
class DownloadWorker : public QObject
{
    Q_OBJECT
public slots:
    void slot_downloadWork( char* buf , int nlen);
};
...
signals:
    ...
    void  SIG_PlayRecord(QString);
    void  SIG_setMovie(int id , QString path , QString rtmp);
    void  SIG_DownloadBlock( char* buf, int nlen);
public slots:
    ...
    void slot_setMovie(int id, QString path, QString rtmp);
    void InitServerIPFromIni();
private:
    Ui::OnlineDialog *ui;
    LoginDialog * m_login;
    QMyTcpClient * m_tcp;
