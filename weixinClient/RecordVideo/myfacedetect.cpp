...
void MyFaceDetect::detectAndDisplay(Mat& frame ,std::vector<Rect> &faces)
{
    ...
    cvtColor( frame, frame_gray, cv::COLOR_BGR2RGB );
   ...
   face_cascade.detectMultiScale( frame_gray, faces, 1.01, 6, 0, Size(100, 100),Size(300, 300) );
...
//        eyes_cascade.detectMultiScale( faceROI, eyes, 1.1, 2, 0 , Size(30, 30) );
        ...
        nose_cascade.detectMultiScale( faceROI, nose, 1.1, 2, 0 , Size(30, 30) ,Size(300, 300) );
        ...
        {
            if( nose[0].width * 2.5 > rct.width*1.0 )
            {
                ite = faces.erase(ite);
                continue;
            }
